...
If Mechanism C is active&mdash;if the narrative is genuinely acting as an active causal structure rather than just shifting local semantic weights&mdash;it acts as a common confounder $Z$ that couples the outputs: $$P(Y_A, Y_B \mid Z) \neq P(Y_A \mid Z) P(Y_B \mid Z)$$
