...
::: epigraph
What if the coin could land, not heads or tails, but somewhere in between---and from behind two curtains simultaneously?
...
The Rosencrantz construction assigns to each board $B$ a Hilbert space $\mathcal{H}_B = \mathbb{C}^K$ with orthonormal basis $\{|\omega\rangle\}_{\omega \in \Omega_B}$, one vector per valid configuration [@Baldo2026Residue]. The space is finite-dimensional and the state is always a uniform superposition, but the geometric structure is not trivial. The projectors $\Pi_i^M$ and $\Pi_i^S$ decompose $\mathcal{H}_B$ into orthogonal subspaces whose dimensionality ratios *are* the Born-rule probabilities. Lüders projection chains navigate this subspace geometry: each click moves the state to a lower-dimensional subspace. The Hilbert space does exactly what it does in the measurement fragment---it converts geometric facts (subspace dimensions) into probabilities. The restriction is: uniform amplitudes, a preferred basis, finite dimension.
...
The state space is $\mathcal{H}= L^2(T)$, the space of square-integrable functions on the tile. After clearings that leave region $R$, the state is $$\label{eq:continuous-state}
  ...
  \mathrm{Pr}[\text{mine in } S] = \int\!\!\int_{S \cap R} |\psi(x,y)|^2 \, dx \, dy = \frac{\mathrm{Area}(S \cap R)}{\mathrm{Area}(R)}$$ for any measurable subset $S \subseteq T$. The probability is an area ratio---geometry does the work that counting did in the discrete case.
...
GoL is many-to-one: multiple initial configurations can evolve to the same final configuration. When we collect terms in [\[eq:gol-evolution\]](#eq:gol-evolution){reference-type="eqref" reference="eq:gol-evolution"}: $$\label{eq:nonuniform}
  |\psi_t\rangle = \sum_{\omega'} \frac{n(\omega')}{\sqrt{K}} |\omega'\rangle,$$ where $n(\omega')$ counts the number of initial configurations that evolved to $\omega'$. The amplitudes are no longer uniform. The Born rule no longer reduces to simple counting---it is weighted by predecessor multiplicity: $$\label{eq:gol-born}
...
\end{aligned}$$ where $w$ is the slit width and $x_2 - x_1$ is the slit separation. The cleared region between and around the strips is acquired knowledge. The two strips are the remaining ignorance. The mine---which does not yet exist---is in superposition over $R_1 \cup R_2$.
...
**Run B (slit 1 only):** Clear everything except $R_1$ (close slit 2 by clearing it---acquiring that knowledge). Evolve $t$ steps. Probe screen. Record $P_1(x)$.
...
::: center
  **Feature**            **Discrete $H{=}0$**    **$+$ Continuous**          **$+$ GoL**          **Full QM**
  -------------------- ------------------------ -------------------- --------------------------- --------------
  Hilbert space         $\mathbb{C}^K$, finite   $L^2(\text{Tile})$   $+$ non-trivial evolution   $\checkmark$
...
This recovers position-momentum complementarity: measurements in the position basis and the Fourier basis are non-commuting, and the ground truth in the Fourier basis has genuine oscillatory structure---not because of complex amplitudes in the position basis, but because the *change of basis* introduces complex phases.
...
The deepest open question at the interference frontier: does there exist a *natural, playable* game---one with simple, intuitive rules that a non-physicist could learn---whose ground truth requires complex amplitudes?
